                ...
                <div class="card">
                    <div class="card-header">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            ...
                            <h3>{{ $users->count() }} Docentes Registrados</h3>
                                    ...
                                    <tr>
                                        <td style="width: 30%">
                                            <div style="font-size: 1px">
                                                filt_{{ $user->prg }}_{{ $user->ciclo }}_{{ substr($user->grupo, -1) }}
                                            </div>
                                        ...
                                        <td style="width: 30%">{{ $user->telefono }}<br>{{ $user->email }}
                                            <br>{{ $user->tipoDoc }} {{ $user->doc }} de
                                            {{ $user->doc_ex }}<br>{{ $user->fecha_nac }}<br>Cód: {{ $user->cod }}
                                        </td>
                                        <td style="width: 20%">
                                            <form class="row" action="{{ route('users.destroy', $user->id) }}"
                                                method="POST">
                                                <a class="col-4 btn btn-sm btn-info btwToolt"
                                                    href="/inbox/{{ $user->cod }}" data-toggle="tooltip"
                                                    data-placement="top" title="Escribir"><i
                                                        class="fa-solid fa-square-envelope"></i></a>
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="col-4 btn btn-danger btn-sm btwToolt"
                                                    onclick="return confirm('¿Desea deshabilitar este usuario?')"
                                                    data-toggle="tooltip" data-placement="top" title="Deshabilitar"><i
